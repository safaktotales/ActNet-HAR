...
from tensorflow.keras.layers import (
    ...
    Concatenate, GlobalMaxPooling2D, Dropout, Dense, Lambda,
    Reshape, Activation, Multiply, Add
)
...
def channel_attention(inputs):
    ...
    x = Conv2D(shape[3] // 8, 1, padding='same', use_bias=False)(x)
    x = Activation('relu')(x)
    x = Conv2D(shape[3], 1, padding='same', use_bias=False)(x)
    x = Activation('sigmoid')(x)
    x = Multiply()([x, inputs])
    return x

def build_actnet(input_shape=(224, 224, 3), num_classes=2):
    K.clear_session()
